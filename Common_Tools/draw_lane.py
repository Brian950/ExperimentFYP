...
class DrawLane:
    ...
    def validate_lane(self, img, left_lane_inds, right_lane_inds):
        ...
        if len(self.left_line_allx) <= 500 or len(self.right_line_allx) <= 500:
    ...
    def assemble_img(self, warped, polynomial_img, lane_img, combo_image):
        ...
        boxsize, _ = cv2.getTextSize("Threshold & Hough Lines", fontFace, fontScale, thickness)
        cv2.putText(img_out, "Threshold & Hough Lines", (int(1080 - boxsize[0] / 2), 280), fontFace, fontScale, (255, 255, 255),
                    thickness, lineType=cv2.LINE_AA)
